...
import { getReparacionesPorMarca } from 'src/services/reparacionService';

const GraficaReparacionesPorMarca = () => {
  ...
  const actualizarGrafica = async () => {
    ...
    try {
      const response = await getReparacionesPorMarca(
        rangoFechas[0].toISOString().split('T')[0],
        rangoFechas[1].toISOString().split('T')[0]
      );

      const categories = response.data.map((item) => item._id); // Nombres de las marcas
...
export default GraficaReparacionesPorMarca;
